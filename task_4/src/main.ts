...
import type { ChatMessage, ChatSettings, SavedChat, SavedChatMetadata, TokenUsage, UiChatMessage } from "./types";
...
function getSavedTokenUsage(metadata: SavedChatMetadata): TokenUsage {
  return metadata.tokenUsage || createEmptyTokenUsage();
}
...
function applySavedMetadata(metadata: SavedChatMetadata) {
  ...
  currentTokenUsage = getSavedTokenUsage(metadata);
  syncSettingsForm();
}
...
function renderSavedChats() {
  ...
  savedDialogsContainer.innerHTML = savedChats
    .map((chat) => {
      ...
      const tokenUsage = getSavedTokenUsage(chat.metadata);
      ...
      return `
        ...
        >
          ...
          <small>${escapeHtml(chat.metadata.model)} · ${formatTokenCount(tokenUsage.totalTokens)} токенов</small>
        </button>
      `;
    })
    .join("");
...
function renderSettingsMetadata(value: ChatSettings | SavedChatMetadata, tokenUsage: TokenUsage) {
